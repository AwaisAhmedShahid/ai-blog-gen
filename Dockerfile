...
RUN \
  if [ -f yarn.lock ]; then yarn db:gen && yarn run build; \
  elif [ -f package-lock.json ]; then npm run db:gen && npm run build; \
  elif [ -f pnpm-lock.yaml ]; then corepack enable pnpm && pnpm run db:gen && pnpm run build; \
  else echo "Lockfile not found." && exit 1; \
  fi
...
# Copy all the files and folders except .next
